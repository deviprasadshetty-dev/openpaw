...
impl Tool for FileEditTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolResult> {
        ...
        let mut file = match fs::File::create(&resolved) {
             Ok(f) => f,
             Err(e) => return Ok(ToolResult::fail(format!("Failed to open file for writing: {}", e))),
        };
        ...
        Ok(ToolResult::ok(format!("Replaced {} bytes with {} bytes in {}", old_text.len(), new_text.len(), path_str)))
    }
}
