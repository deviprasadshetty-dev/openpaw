...
impl Tool for BrowserOpenTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolResult> {
        ...
        let host = url.split("://").nth(1).unwrap_or("").split('/').next().unwrap_or("").split(':').next().unwrap_or("");
        ...
        if self.allowed_domains.is_empty() {
             return Ok(ToolResult::fail("No allowed_domains configured for browser_open"));
        }
        ...
        return Ok(ToolResult::fail("browser_open not supported on this platform"));
...
fn is_local_or_private(host: &str) -> bool {
    host == "localhost" || host.ends_with(".localhost") || host.ends_with(".local") || host == "::1" ||
    host.starts_with("127.") || host.starts_with("10.") || host.starts_with("192.168.") || host.starts_with("169.254.")
}

fn host_matches_allowlist(host: &str, allowed: &[String]) -> bool {
    for domain in allowed {
        if host == domain { return true; }
        if host.ends_with(domain) && host.len() > domain.len() && host.as_bytes()[host.len() - domain.len() - 1] == b'.' {
             return true;
        }
    }
    false
